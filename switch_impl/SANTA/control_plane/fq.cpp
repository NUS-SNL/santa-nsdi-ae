...
#ifndef PROG_NAME
#error "Please add -DPROG_NAME=\"fq\" to CPPFLAGS"
#endif

#define CONF_FILE_DIR        "share/p4/targets/tofino"
#define CONF_FILE_PATH(prog) \
    SDE_INSTALL "/" CONF_FILE_DIR "/fq.conf"
...
bf_status_t app_run(bf_switchd_context_t *switchd_ctx)
{
    ...
    fflush(stdout);
    std::string bfshell_cmd = BFSHELL " -b " __DIR__ "/setup_scripts/setup_tofino_fq.py";
    ...
    std::string run_pd_rpc_cmd = "env PYTHONPATH=" + customPythonPath + " SDE_INSTALL=" SDE_INSTALL " " RUN_PD_RPC " " __DIR__ "/setup_scripts/set_rate_fq.py";
