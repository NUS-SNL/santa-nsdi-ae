...
launch_three_flows() {
    ...
    echo "Launching 3 flows for CCA: ${cca} with RTT: ${rtt}ms"
    ...
    for i in {0..2}; do
        ...
        sudo ip netns exec recv tcpdump -i ens2f1 tcp and dst port $port -w ~/eval/pcap_9_scatter_2/recv_${cca}_${rtt}ms_flow${i}.pcap &
        # capturing on the sender side for packets going to the receiver ports
        sudo ip netns exec send tcpdump -i ens2f0 tcp and dst port $port -w ~/eval/pcap_9_scatter_2/send_${cca}_${rtt}ms_flow${i}.pcap &
...
mkdir -p ~/eval/pcap_9_scatter_2
sudo sysctl net.ipv4.tcp_timestamps=0

disable_tso_gro
sudo tcpdump -i ens1f0 tcp -w ~/eval/pcap_9_scatter_2/total_drop.pcap &
