...
launch_two_flows() {
    ...
    for i in {0..1}; do
        ...
        sudo ip netns exec recv tcpdump -i ens2f1 tcp and dst port $port -w ~/eval/pcap_6_flow_santa_6q/recv_${cca}_${rtt}ms_flow${i}.pcap &
        # capturing on the sender side for packets going to the receiver ports
        sudo ip netns exec send tcpdump -i ens2f0 tcp and dst port $port -w ~/eval/pcap_6_flow_santa_6q/send_${cca}_${rtt}ms_flow${i}.pcap &
...
mkdir -p ~/eval/pcap_6_flow_santa_6q
sudo sysctl net.ipv4.tcp_timestamps=0

disable_tso_gro
sudo tcpdump -i ens1f0 tcp -w ~/eval/pcap_6_flow_santa_6q/total_drop.pcap &

launch_two_flows vegas 20 7000 12000
launch_two_flows bbr 20 6000 11000
launch_two_flows cubic 20 5000 10000
