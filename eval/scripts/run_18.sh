...
launch_six_flows() {
    ...
    for i in {0..5}; do
        ...
        sudo ip netns exec recv tcpdump -i ens2f1 tcp and dst port $port -w ~/eval/pcap_18_flow_santa/recv_${cca}_${rtt}ms_flow${i}.pcap &
        # capturing on the sender side for packets going to the receiver ports
        sudo ip netns exec send tcpdump -i ens2f0 tcp and dst port $port -w ~/eval/pcap_18_flow_santa/send_${cca}_${rtt}ms_flow${i}.pcap &
...
mkdir -p ~/eval/pcap_18_flow_santa
sudo sysctl net.ipv4.tcp_timestamps=0

disable_tso_gro
sudo tcpdump -i ens1f0 tcp -w ~/eval/pcap_18_flow_santa/total_drop.pcap &

launch_six_flows vegas 20 7000 12000
launch_six_flows bbr 20 6000 11000
launch_six_flows cubic 20 5000 10000
